                ...
                <div class="row breadcrumbs-top">
                    <div class="breadcrumb-wrapper col-12">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="{{ route('dashboard.countries.index') }}">المناطق</a>
                            </li>
                            <li class="breadcrumb-item"><a href="javascript:;">تعديل منطقة - {{ $country->name }}</a>
                            </li>
                        </ol>
                    </div>
                </div>
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">بيانات منطقة - {{ $country->name }}</h4>
                            ...
                            <form class="form-horizontal" action="{{ url('dashboard/countries/update') }}" method="post" novalidate enctype="multipart/form-data">
                                @csrf
                                        ...
                                        <div class="row match-height">
                                            <div class="col-xl-6 col-lg-6 col-md-12">
                                                <fieldset class="form-group">
                                                    <label class="card-title success" for="inputSuccess">أسم المنطقة<code>*</code></label>
                                                    <input type="text" class="form-control border-success info" name="name"
                                                        id="name"
                                                    @if(empty($country->name)) value="{{ old('name') }}"
                                                    @else
                                                    value="{{ $country->name }}"
                                                    @endif
                                            ...
                                            <div class="col-xl-6 col-lg-6 col-md-12">
                                                <fieldset class="form-group">
                                                    <label class="card-title success" for="inputSuccess">حالة المنطقة<code>*</code></label>
                                                    <select name="status" id="status"
                                                        class="select2 form-control border-success info">
                                                        <option value="">حدد حالة </option>
                                                        <option value="1" @if($country->status==1) selected @endif>مفعل</option>
                                                        <option value="0" @if($country->status==0) selected @endif>غير مفعل</option>
                                                    </select>
                                                    @error("status")
                                                    <p class="badge-default badge-danger block-tag text-center">
                                                        <small class="block-area white">{{$message}}</small>
